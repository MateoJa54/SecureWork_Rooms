...
export default function SalaChat() {
  ...
  useEffect(() => {
    if (!sessionToken || String(salaIdGuardada) !== String(id)) {
  ...
  }, [id, sessionToken, salaIdGuardada]);

  const handleSend = useCallback(
    (contenido) => {
      if (!socketRef.current) return;

      socketRef.current.emit('mensaje:enviar', {
        sala_id: id,
        contenido
      });
    },
    ...
    <div className="h-screen flex flex-col bg-gray-50">
      <header className="bg-white border-b border-gray-200 px-4 py-3 flex items-center justify-between shrink-0">
        <div className="flex items-center gap-3">
          <span
            className={`h-2 w-2 rounded-full ${
              conectado ? 'bg-green-400' : 'bg-gray-300'
            }`}
          />
        ...
        <div className="flex items-center gap-4">
          {sala?.tipo_sala === 'multimedia' && socketRef.current && (
            <FileUpload
              salaId={id}
              sessionToken={sessionToken}
              maxMb={sala.tamanio_max_archivo_mb}
              onUploaded={(f) =>
                socketRef.current?.emit('archivo:notificar', {
                  archivo_id: f.id
                })
              }
            />
